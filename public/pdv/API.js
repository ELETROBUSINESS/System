...
function listarTodosProdutos() {
    Logger.log("Executando listarTodosProdutos (Versão Fiscal 15 Colunas)...");

    try {
        const sheet = getSheet(PRODUTOS_SHEET_NAME);

        // O formatSheetData lê a planilha e cria objetos com chaves em minúsculo
        // Ex: Coluna "ImgUrl" vira item.imgurl
        ...
        const produtos = data.map(item => {
            // Tratamento de Preço (garante que seja número)
            let precoNum = 0;
            if (typeof item.preco === 'number') {
                precoNum = item.preco;
            } else if (item.preco) {
                // Tenta converter texto "R$ 10,00" ou "10,00" para 10.00
                let pString = String(item.preco).replace("R$", "").trim().replace(",", ".");
                precoNum = parseFloat(pString) || 0;
            }

            // --- MAPEAMENTO EXATO DA SUA PLANILHA (15 COLUNAS) ---
            return {
                // Dados Básicos
                ...
                imgUrl: String(item.imgurl || ""), // 'ImgUrl' vira 'imgurl'
                brand: String(item.marca || ""),

                // Dados Fiscais (NFC-e)
                ncm: String(item.ncm || ""),       // Garante string para evitar erro
        ...
        }).filter(p => p.id && p.id !== ""); // Remove linhas vazias se houver

        Logger.log("Produtos processados com sucesso: " + produtos.length);
...
function saveProduct(data) {
    ...
    // Procura se o produto já existe pelo ID (Coluna A)
    for (let i = 1; i < values.length; i++) {
        if (String(values[i][0]) == String(data.id)) {
            rowIndex = i + 1; // +1 pois array começa em 0 e planilha em 1
            break;
        }
    }

    // Monta a linha de dados (15 colunas) - Respeitando colunas antigas A-G
    const rowData = [
        data.id,                              // A: codigo
        data.name,                            // B: Nome
        data.price || 0,                      // C: Preco
        new Date(),                           // D: Timestamp
        data.costPrice || 0,                  // E: Custo
        data.category || 'Geral',             // F: Categoria
        'TRUE',                               // G: Cadastrado
    ...
    if (rowIndex > 0) {
        // Edição: Atualiza apenas a linha encontrada (15 colunas)
        sheet.getRange(rowIndex, 1, 1, 15).setValues([rowData]);
        return { status: "success", message: "Produto atualizado com dados fiscais!", type: "edit" };
    } else {
        // Novo: Adiciona linha no final
        sheet.appendRow(rowData);
        return { status: "success", message: "Novo produto cadastrado!", type: "create" };
    }
}
